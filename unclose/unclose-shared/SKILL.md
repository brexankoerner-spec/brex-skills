---
name: unclose
description: Use this skill whenever the user provides a #unclose-at-risk Slack thread link and wants to review, improve, or draft a response. Triggers on any of these: "coach my unclose response," "review this thread," "help me draft a reply," "review my submission," "coach this," or any paste of a podium.slack.com/archives/C07TZG7K7DZ URL. Also trigger proactively if the user pastes a Slack thread link alongside a draft message without explicitly asking for coaching — the intent is clear. Always invoke this skill before the user posts anything to #unclose-at-risk.
---

# Unclose Response Coach

Use this skill when you want to review, improve, or draft a response for a thread in `#unclose-at-risk`. Triggers include: "coach my unclose response," "review this thread," "help me draft a reply," "review my submission," or any time you provide a `#unclose-at-risk` thread link.

---

## Your setup (fill in once after installing)

Replace the placeholders below with your own details. The skill uses these to personalize coaching output and reference your management chain correctly.

```
Your name: [Your name]
Your direct manager: [Manager name]
```

Potts (Robert Potts) is hardcoded as the final approver — he reviews all unclose threads at the approval stage and this applies to the whole team.

---

## What this skill does

Reviews your draft response (or generates one if none is provided) for a `#unclose-at-risk` Slack thread. Returns a clean, ready-to-post revised response plus line-by-line coaching notes explaining every change.

The skill has two modes:
- **INITIAL SUBMISSION** — coaching the bot form fields you fill out when first submitting an account
- **FOLLOW-UP** — coaching a reply you are adding to an already-open thread

You will always label which mode applies.
...
3. Your draft text — OR a request to generate a draft from scratch
...
- The first message is the bot submission notification — this contains the submission type, account name, and the form fields the OM or AE filled out.
...
- Has the OM (you) already posted? If so, what did you say?
...
In this mode, you are drafting the form fields you'll fill out when submitting an account. These fields are management's first impression of the situation — they are reviewed before anyone reads the thread.
...
In this mode, you are drafting a reply in an ongoing thread. These replies are read by management, the OMM, and the AE — they form your visible record of how you handle at-risk accounts.

**What to evaluate in your draft:**
...
2. **Ownership vs. reaction** — Are you posting proactively (volunteering an update) or reactively (responding to being asked)?
...
5. **Tone and length** — Follow-up replies should be concise (1–4 sentences in most cases). Long replies in the middle of a thread suggest the OM is over-explaining or uncertain. If your draft is long, tighten it.

6. **Management load** — Does the reply create work for management (asks a procedural question they shouldn't need to answer, leaves the next step undefined) or absorb it (you own the next move)?

**Context sensitivity:** Use the thread state assessment from Step 5 to calibrate. If management just set a deadline in the thread, your reply should acknowledge it and confirm you own the action. If the thread has gone quiet for a week, the reply should re-energize it with a specific update and timeline.
...
[The clean, ready-to-post version of your draft — or a generated draft if none was provided. For INITIAL SUBMISSION mode, this is the revised form fields. For FOLLOW-UP mode, this is the reply text. Write it in the user's voice — first person, direct, professional. No preamble, no meta-commentary inside this section.]
...
If your draft was already strong in an area, note that too — "Kept X as-is — this is exactly right because..." — so you understand what you did well, not just what needed fixing.]
...
Every reply in a `#unclose-at-risk` thread is part of a record read by two tiers of management:

- **Your direct manager** (as specified in Your Setup) — active participant in threads, primary day-to-day collaborator. They can be addressed directly in replies and will often redirect or add context.
- **Robert Potts ("Potts")** — final approver on all unclose decisions. He reads threads at the approval stage and may opine intermittently on larger accounts or significant product gaps. He is not typically active throughout a thread's full lifespan.
...
The goal is not to make every reply answer all 5 questions — that would be exhausting and unnatural. The goal is to help you build a thread record that, when Potts reads it at the approval stage, allows him to answer all 5 without gaps or ambiguity.
...
If your draft leaves a gap in the thread record that would prevent Potts from answering one of these questions, flag it in the coaching notes — even if the draft itself is otherwise strong.
...
3. **Tighten, don't expand.** If your draft is long, make it shorter. The goal is concise, high-signal communication — not comprehensive documentation in every reply. Reserve detail for the initial submission fields; follow-up replies should be brief.

4. **Never fabricate context.** Only use information present in the thread or your draft. Do not invent account details, customer statements, or outcomes that aren't documented.

5. **Write in the user's voice.** The revised response should sound like you — not a formal template, not a legal brief. Direct, confident, first-person, professional. Avoid overusing em-dashes (—). One per response is fine; more than that reads as AI-generated. Use periods, commas, or sentence breaks instead.

6. **Coaching notes teach, they don't just correct.** Every note should explain *why* a change was made, not just flag what was wrong. You should be able to internalize the principle and apply it the next time without the skill.
...
**Submission type unclear:** State your best interpretation of the type, flag the ambiguity in the coaching notes, and proceed with the most likely type. If truly indeterminate, ask the user to clarify before proceeding.

**Thread is very long (50+ replies):** Summarize the thread state briefly before the output sections so you can confirm the skill understood the situation correctly. Focus the read on the most recent 15–20 replies for current state; read the original submission for type identification.

**User has already posted in the thread:** Note this in the thread state summary. If a previous reply had issues the coaching notes would flag, mention them briefly — but focus the revised response on the new draft, not on re-litigating past posts.

**Management has already directed a specific action:** The revised response should align with that direction, not contradict it. If your draft conflicts with a management directive in the thread, flag this in the coaching notes before presenting the revision.
