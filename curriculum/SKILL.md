...
```bash
uv run ~/.claude/skills/curriculum/scripts/scaffold.py --output-dir <absolute_output_dir>
```
...
If the user has memory tooling configured (auto-memory in `~/.claude/CLAUDE.md`, a memory-graph MCP, etc.), save a user-type memory pointing at the curriculum: the output directory, the topic, the user's goal, and a reminder to read `curriculum-meta.md` before assessing. This enables the Resume workflow in later sessions without re-gathering context. If no memory tooling is configured, skip this step — the `progress.md` file plus the directory structure are sufficient.
...
```bash
echo '<single-line-json>' | uv run ~/.claude/skills/curriculum/scripts/append_assessment.py --curriculum-dir <output_dir>
```
...
```bash
uv run ~/.claude/skills/curriculum/scripts/compute_progress.py --curriculum-dir <output_dir>
```
