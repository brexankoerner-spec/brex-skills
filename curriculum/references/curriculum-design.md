...
Example: if the user asks for a curriculum on "deep learning" and
emphasizes "computer vision," the curriculum might split as:

- Phase 1: 01 Tensors & Autograd, 02 Gradient Descent
- Phase 2: 03 MLPs & Activations, 04 Training Loops & Loss Functions,
  05 Regularization & Optimization
- Phase 3 (emphasis): 06 Convolutions, 07 CNN Architectures,
  08 Object Detection & Segmentation, 09 Modern Vision (ViT, CLIP, diffusion)
- 10 Synthesis

Vs the same "deep learning" curriculum with "sequence models" as emphasis:

- Phase 1–2 the same through 05
- Phase 3: 06 RNNs & LSTMs, 07 Attention & Transformers, 08 LLMs & Fine-Tuning,
  09 Retrieval & Agents
- 10 Synthesis

Same subject, different shape, because emphasis differs.
...
- Goal: "build AI agent self-improvement flywheels" → cut modules on
  generative modeling; keep modules on classification, evaluation,
  feedback loops.
- Goal: "pass the Google data scientist L4 interview" → cut deep-dive
  on observability; keep modules on probability, SQL, product sense,
  A/B testing, classical ML.
- Goal: "build a weather app for my city that beats the local
  forecaster's MAE" → cut modules on classification; keep modules on
  regression, time-series, feature stores, and model monitoring.
